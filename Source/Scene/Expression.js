define([
    ...
    Expression.prototype.evaluate = function(frameState, feature, result) {
        ...
        var value = this._runtimeAst.evaluate(this, frameState, feature);
    ...
    Expression.prototype.evaluateColor = function(frameState, feature, result) {
        scratchStorage.reset();
        var color = this._runtimeAst.evaluate(this, frameState, feature);
    ...
    function evaluateTilesetTime(expression, frameState, feature) {
        return feature.content.tileset.timeSinceLoad;
    }

    function getEvaluateUnaryFunction(call) {
        var evaluate = unaryFunctions[call];
        return function(expression, frameState, feature) {
            var left = this._left.evaluate(expression, frameState, feature);
    ...
    function getEvaluateBinaryFunction(call) {
        ...
        return function(expression, frameState, feature) {
            var left = this._left.evaluate(expression, frameState, feature);
            var right = this._right.evaluate(expression, frameState, feature);
    ...
    function getEvaluateTernaryFunction(call) {
        ...
        return function(expression, frameState, feature) {
            var left = this._left.evaluate(expression, frameState, feature);
            var right = this._right.evaluate(expression, frameState, feature);
            var test = this._test.evaluate(expression, frameState, feature);
            return evaluate(call, left, right, test);
        };
    }

    Node.prototype._evaluateLiteral = function(expression, frameState, feature) {
        return this._value;
    };

    Node.prototype._evaluateLiteralColor = function(expression, frameState, feature) {
        ...
        if (this._value === 'color') {
            if (!defined(args)) {
                ...
                Color.fromCssColorString(args[0].evaluate(expression, frameState, feature), color);
                color.alpha = args[1].evaluate(expression, frameState, feature);
            } else {
                Color.fromCssColorString(args[0].evaluate(expression, frameState, feature), color);
            }
        } else if (this._value === 'rgb') {
            Color.fromBytes(
                args[0].evaluate(expression, frameState, feature),
                args[1].evaluate(expression, frameState, feature),
                args[2].evaluate(expression, frameState, feature),
                255, color);
        } else if (this._value === 'rgba') {
            // convert between css alpha (0 to 1) and cesium alpha (0 to 255)
            var a = args[3].evaluate(expression, frameState, feature) * 255;
            Color.fromBytes(
                args[0].evaluate(expression, frameState, feature),
                args[1].evaluate(expression, frameState, feature),
                args[2].evaluate(expression, frameState, feature),
            ...
            Color.fromHsl(
                ...
                args[1].evaluate(expression, frameState, feature),
                args[2].evaluate(expression, frameState, feature),
                args[3].evaluate(expression, frameState, feature),
                color);
        }
        return Cartesian4.fromColor(color, scratchStorage.getCartesian4());
    };

    Node.prototype._evaluateLiteralVector = function(expression, frameState, feature) {
        ...
        var argsLength = args.length;
        for (var i = 0; i < argsLength; ++i) {
            var value = args[i].evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateLiteralString = function(expression, frameState, feature) {
        return this._value;
    };

    function getMutable(expression, variableName) {
        var mutables = expression._mutables;
        var mutable = mutables[variableName];
        if (defined(mutable)) {
    ...
    Node.prototype._evaluateVariableString = function(expression, frameState, feature) {
        var result = this._value;
        var match = variableRegex.exec(result);
        while (match !== null) {
            var placeholder = match[0];
            var variableName = match[1];
            var mutable = getMutable(expression, variableName);
            if (defined(mutable)) {
                // TODO : check this
                return mutable;
            }
    ...
    Node.prototype._evaluateVariable = function(expression, frameState, feature) {
        var mutable = getMutable(expression, this._value);
        if (defined(mutable)) {
            return mutable;
        }
    ...
    Node.prototype._evaluateMemberDot = function(expression, frameState, feature) {
        if (checkFeature(this._left)) {
            return feature.getProperty(this._right.evaluate(expression, frameState, feature));
        }
        var property = this._left.evaluate(expression, frameState, feature);
        if (!defined(property)) {
            return undefined;
        }

        var member = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateMemberBrackets = function(expression, frameState, feature) {
        if (checkFeature(this._left)) {
            return feature.getProperty(this._right.evaluate(expression, frameState, feature));
        }
        var property = this._left.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateArray = function(expression, frameState, feature) {
        var array = [];
        for (var i = 0; i < this._value.length; i++) {
            array[i] = this._value[i].evaluate(expression, frameState, feature);
        }
        return array;
    };
    ...
    Node.prototype._evaluateNot = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateNegative = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluatePositive = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateLessThan = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateLessThanOrEquals = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateGreaterThan = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateGreaterThanOrEquals = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateOr = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        if (typeof left !== 'boolean') {
    ...
    Node.prototype._evaluateAnd = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        if (typeof left !== 'boolean') {
            throw new RuntimeError('Operator "&&" requires boolean arguments. First argument is ' + left + '.');
        }
    ...
    Node.prototype._evaluatePlus = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateMinus = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2)) {
    ...
    Node.prototype._evaluateTimes = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2)) {
    ...
    Node.prototype._evaluateDivide = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2)) {
    ...
    Node.prototype._evaluateMod = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2)) {
    ...
    Node.prototype._evaluateEqualsStrict = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2) ||
    ...
    Node.prototype._evaluateNotEqualsStrict = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
        if ((right instanceof Cartesian2) && (left instanceof Cartesian2) ||
            (right instanceof Cartesian3) && (left instanceof Cartesian3) ||
    ...
    Node.prototype._evaluateConditional = function(expression, frameState, feature) {
        var test = this._test.evaluate(expression, frameState, feature);
        ...
        if (test) {
            return this._left.evaluate(expression, frameState, feature);
        }
        return this._right.evaluate(expression, frameState, feature);
    };

    Node.prototype._evaluateNaN = function(expression, frameState, feature) {
        return isNaN(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateIsFinite = function(expression, frameState, feature) {
        return isFinite(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateIsExactClass = function(expression, frameState, feature) {
        return feature.isExactClass(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateIsClass = function(expression, frameState, feature) {
        return feature.isClass(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluategetExactClassName = function(expression, frameState, feature) {
        return feature.getExactClassName();
    };

    Node.prototype._evaluateBooleanConversion = function(expression, frameState, feature) {
        return Boolean(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateNumberConversion = function(expression, frameState, feature) {
        return Number(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateStringConversion = function(expression, frameState, feature) {
        return String(this._left.evaluate(expression, frameState, feature));
    };

    Node.prototype._evaluateRegExp = function(expression, frameState, feature) {
        var pattern = this._value.evaluate(expression, frameState, feature);
        var flags = '';

        if (defined(this._left)) {
            flags = this._left.evaluate(expression, frameState, feature);
        }
    ...
    Node.prototype._evaluateRegExpTest = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateRegExpMatch = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateRegExpNotMatch = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateRegExpExec = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
        var right = this._right.evaluate(expression, frameState, feature);
    ...
    Node.prototype._evaluateToString = function(expression, frameState, feature) {
        var left = this._left.evaluate(expression, frameState, feature);
